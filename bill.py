...
class Bill_App:
    ...
    def create_ui(self):
        ...
        self.c_name = st.text_input("Name", key="c_name")
        self.c_phone = st.text_input("Phone Number", key="c_phone")
        self.search_bill = st.text_input("Bill Number", key="search_bill")
        if st.button("Search"):
            self.find_bill()
        ...
        with col1:
            ...
            self.items["sanitizer"] = st.number_input("Sanitizer", min_value=0, key="sanitizer")
            self.items["mask"] = st.number_input("Mask", min_value=0, key="mask")
            self.items["hand_gloves"] = st.number_input("Hand Gloves", min_value=0, key="hand_gloves")
            self.items["dettol"] = st.number_input("Dettol", min_value=0, key="dettol")
            self.items["newsprin"] = st.number_input("Newsprin", min_value=0, key="newsprin")
            self.items["thermal_gun"] = st.number_input("Thermal Gun", min_value=0, key="thermal_gun")

        with col2:
            st.header("Grocery Items")
            self.items["rice"] = st.number_input("Rice", min_value=0, key="rice")
            self.items["food_oil"] = st.number_input("Food Oil", min_value=0, key="food_oil")
            self.items["wheat"] = st.number_input("Wheat", min_value=0, key="wheat")
            self.items["daal"] = st.number_input("Daal", min_value=0, key="daal")
            self.items["flour"] = st.number_input("Flour", min_value=0, key="flour")
            self.items["maggi"] = st.number_input("Maggi", min_value=0, key="maggi")

        with col3:
            st.header("Cold Drinks")
            self.items["sprite"] = st.number_input("Sprite", min_value=0, key="sprite")
            self.items["limka"] = st.number_input("Limka", min_value=0, key="limka")
            self.items["mazza"] = st.number_input("Mazza", min_value=0, key="mazza")
            self.items["coke"] = st.number_input("Coke", min_value=0, key="coke")
            self.items["fanta"] = st.number_input("Fanta", min_value=0, key="fanta")
            self.items["mountain_duo"] = st.number_input("Mountain Duo", min_value=0, key="mountain_duo")

        if st.button("Total"):
            self.total()
        if st.button("Generate Bill"):
            self.total()  # Ensure total is calculated before generating the bill
            self.bill_area()
        if st.button("Clear"):
            self.clear_data()
        if st.button("Exit"):
    ...
    def total(self):
        ...
        st.success(f"Total Bill: Rs. {self.total_bill}")
